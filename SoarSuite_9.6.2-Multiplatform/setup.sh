...
linux64_setup () {
  ...
  cp "$SOAR_HOME/linux_x86-64/swt.jar" "$SOAR_HOME/java/"
  cp "$SOAR_HOME/linux_x86-64"/* "$SOAR_HOME/"
...
if [ ! -e "$SOAR_HOME/pkgIndex.tcl" ]; then
  ...
  if [[ "$unamestr" == 'Linux' ]]; then
    ...
    if [ "$(uname -m)" == "arm64" ]; then
      ...
      cp "$SOAR_HOME/mac_ARM64/swt.jar" "$SOAR_HOME/java/"
      cp "$SOAR_HOME/mac_ARM64"/* "$SOAR_HOME/"
    else
      echo_yellow 'First time initialization of Soar for Mac OSX x86-64...'
      cp "$SOAR_HOME/mac_x86-64/swt.jar" "$SOAR_HOME/java/"
      cp "$SOAR_HOME/mac_x86-64"/* "$SOAR_HOME/"
    fi
      macOS_quarantine_fix
  else
